...
{
	if (__menu_state == BATTLE_MENU_STATE.BUTTON_SELECTION)
	{
		var _button_slot = __menu_button_choice,
			_curButton = Battle.__defined_buttons[$ Button[_button_slot]];
		//Changing button choices
		if (input_horizontal != 0)
		{
			_button_slot = posmod(_button_slot + input_horizontal, array_length(Button));
			__menu_button_choice = _button_slot;
			audio_play(snd_menu_switch);
			struct_foreach(Battle.__defined_buttons, function(_name, _value) {
				_value.__ColorLerpTimer = 0;
			});
		}
		//Soul lerping
		with (oSoul)
		{
			visible = true;
			x = decay(x, _curButton.x + _curButton.soul_x * _curButton.image_xscale, lerp_speed);
			y = decay(y, _curButton.y + _curButton.soul_y * _curButton.image_yscale, lerp_speed);
		}
		//If input is detected, change state to button state
		if (input_confirm)
		{
			audio_play(snd_menu_confirm);
			__menu_state = _curButton.state;
			//If target state is item and there are no items left, return to button selection state
			if (__menu_state == BATTLE_MENU_STATE.ITEM && __item_count == 0)
			{
				__menu_state = BATTLE_MENU_STATE.BUTTON_SELECTION;	
				if (ItemMenuScrollType == ITEM_SCROLL.VERTICAL)
					__menu_choices[BATTLE_MENU_STATE.ITEM] = 0;
	...
	if (ChangeSoulAngle && is_val(__menu_state, BATTLE_MENU_STATE.FIGHT, BATTLE_MENU_STATE.ACT, BATTLE_MENU_STATE.ITEM, BATTLE_MENU_STATE.MERCY, BATTLE_MENU_STATE.ACT_SELECT))
