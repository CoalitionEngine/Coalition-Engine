...
else if (state == 2)
{
	...
	if (alpha == 1)
	{
		...
		if (gameover_typist.get_paused())	
			gameover_typist.unpause();

		if (gameover_typist.get_state() == 1 && PRESS_CONFIRM && gameover_writer.get_page() < (gameover_writer.get_page_count() - 1))
			gameover_writer.page(gameover_writer.get_page() + 1);
		else if (PRESS_CONFIRM && gameover_typist.get_state() == 1 && gameover_writer.get_page() >= (gameover_writer.get_page_count() - 1))
		{
			state++;
