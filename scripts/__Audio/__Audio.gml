...
///@return {Id.Sound} The sound you played
...
///@func AudioStickToTime(audio, time, [margin])
...
///@func audio_create_stream_array(names...)
...
function audio_create_stream_array()
{
	...
	for(var i = 0, arr = []; i < argument_count; ++i)
	{
		...
		var text = $"Music/{argument[i]}.ogg";
		array_push(arr, audio_create_stream(text));
	}
	return arr;
}
///@func audio_destroy_stream_array(array)
...
///@func audio_transition(inital_audio, target_audio, time, [single], [loop], [volume], [pitch], [position])
///@desc Transitions an audio to another using fade in/out
...
///@param {real} pitch The pitch of the audio (Default 1)
///@param {real} position The position to play the audio in seconds (Not 3D position, audio position)
//@return {Id.Sound} The sound after transition
